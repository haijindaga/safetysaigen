...
cd core-safety
pip install -r requirements-isaac.txt    # 初回のみ (numpy==1.26.0 を維持する構成)
...
conda activate env_isaaclab
cd core-safety
pip install -r requirements-isaac.txt   # numpy==1.26.0 / osqp ピンを維持
...
壊れた場合は `pip install "numpy==1.26.0" "osqp==0.6.7.post3"
"opencv-python==4.10.0.84"` で復旧する。
...
1. **差動二輪 (Jetbot)** — `scripts/isaac_mobile_demo.py` ✅ 動作確認済み。
2. **四足 (Spot)** — `scripts/isaac_quadruped_demo.py`。
   `isaacsim.robot.policy.examples` の平地歩行ポリシー
   (SpotFlatTerrainPolicy) が速度指令 [vx, vy, ω] を受けるので、
   CBF はその指令をフィルタするだけ。physics 500 Hz / 指令 50 Hz。
3. **モバイルマニピュレータ (Ridgeback+Franka)** —
   `scripts/isaac_manipulator_demo.py` (実験的)。標準アセットの仮想
   ベース関節 (prismatic x/y + revolute z) を速度モードで駆動、
   アームはデフォルト姿勢を保持。起動時に DOF 名一覧を印字するので、
   関節名が違ったらスクリプト冒頭の BASE_JOINT_HINTS を合わせる。
   将来拡張: EE 位置にも同じバリアを適用 (h を EE 平面位置で評価し、
   ベース+アーム合成ヤコビアンで QP を組む)。

## 速度証明書 (Theorem 1 相当)

実測レイテンシが揃ったら最大安全速度を計算する:

```bash
# 例: VLM+セグ+接地の1サイクルが5秒、検出率下界75%の場合
python scripts/compute_certificate.py --latency 5.0 --detection 0.75
```

出た max safe speed を各デモの V_MAX に設定すれば、
P[unsafe進入] <= delta の確率保証つき走行になる。
